...
def lab1_2_handle(req):
	...
	if i==5:
	    ...
	    if math.fabs(odom_theta-theta)<math.pi:
	        ...
	        while new_difference<=previous_difference or math.fabs(new_difference-previous_difference)>0.01:
	    ...
	    while new_distance<=previous_distance or math.fabs(new_distance-previous_distance)>0.01:
	...
	while new_difference<=previous_difference or math.fabs(new_difference-previous_difference)>0.00001:
	    previous_difference=new_difference
 	    new_difference=math.fabs(odom_theta-theta)
	    rospy.sleep(0.01)
	...
	while new_distance<=previous_distance or math.fabs(new_distance-previous_distance)<0.001:
	    #drugi warunek to zabezpieczenie przed szumami pomiarowymi
	    previous_distance=new_distance
	    new_distance=math.sqrt(math.pow(pose.position.x-odom_x,2)+math.pow(pose.position.y-odom_y,2))
	    rospy.sleep(0.01)
...
def lab1_2_server():
    ...
    sub_path = rospy.Subscriber('global_planner/planner/plan', Path,lab1_2_handle)
