...
def lab1_2_handle(req):
	...
	pom_s=math.fabs(odom_theta-theta)
	pom_n=pom_s
	print "pom", pom_n

	while pom_n<=pom_s or math.fabs(pom_n-pom_s)<0.0001:
	    ...
	    if odom_theta<0 and theta<0:
 	        pom_n=math.fabs(odom_theta-theta)
	    elif odom_theta>0 and theta>0:
 	        pom_n=math.fabs(odom_theta-theta)
   	    else:
	    	pom_n=math.fabs(odom_theta+theta)
	...
	if math.fabs(odom_theta-theta)>0.01:
	    rospy.sleep(0.1)
	    pom_s=math.fabs(odom_theta-theta)
	    pom_n=pom_s
	    print "ifffff", pom_n
	    ...
	    while pom_n<=pom_s or math.fabs(pom_n-pom_s)<0.0001:
	        ...
	        print "iffffwhile n", pom_n
	        print "ifffffwhile s", pom_s
	        pom_s=pom_n
	    	#pom_n=math.fabs(odom_theta-theta)
